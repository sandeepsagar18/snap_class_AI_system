...
import { Sparkles, ArrowRight, School, GraduationCap, Lock, User, Mail, Hash, Calendar, Layers, GitBranch, Users, Loader2, ShieldCheck, AlertCircle } from 'lucide-react'
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] flex flex-col justify-center items-center px-4 py-12">
      <div className="text-center max-w-2xl mb-8 relative">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-indigo-500/15 border border-indigo-500/30 text-indigo-300 text-xs font-semibold mb-4 shadow-sm shadow-indigo-500/10">
        ...
        <p className="text-sm sm:text-base text-slate-300 max-w-xl mx-auto leading-relaxed">
          Seamless facial recognition, dynamic QR self-registration, and live attendance tracking with institutional control.
        </p>
      </div>

      <div className={`w-full ${isStudentRegister ? 'max-w-xl' : 'max-w-md'} glass-panel rounded-3xl p-6 sm:p-8 border border-slate-700/60 shadow-2xl relative transition-all duration-300 bg-slate-900/80`}>
        {/* Portal switch tabs */}
        <div className="grid grid-cols-2 gap-2 p-1.5 bg-slate-950/70 rounded-2xl border border-slate-800/90 mb-6">
          <button
            type="button"
        ...
        {error && (
          <div className="mb-4 p-3.5 rounded-2xl bg-rose-500/15 border border-rose-500/30 text-rose-200 text-xs flex items-start gap-2.5">
        ...
        {role === 'teacher' ? (
          <form onSubmit={handleTeacherAuth} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-2 flex items-center justify-center gap-2 py-3 rounded-xl bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold text-sm transition-all shadow-lg shadow-indigo-600/30 cursor-pointer disabled:opacity-50"
            >
              {loading ? (
          ...
          <form onSubmit={handleStudentAuth} className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="text-xs font-medium text-indigo-400 hover:text-indigo-300 transition-colors cursor-pointer shrink-0"
              >
            ...
            {isStudentRegister ? (
              <div className="space-y-3.5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-950/80 border border-slate-700/80 rounded-xl pl-10 pr-3.5 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                        required
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-950/80 border border-slate-700/80 rounded-xl pl-10 pr-3.5 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 font-mono"
                        required
                      />
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950/80 border border-slate-700/80 rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                    />
                  </div>
                  <div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1.5">Live Face Photo Capture (Biometrics)</label>
              ...
              <div className="space-y-4">
                ...
                <div className="p-3 rounded-2xl bg-slate-950/60 border border-slate-800 text-[11px] text-slate-300 flex items-center gap-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-3 flex items-center justify-center gap-2 py-3 rounded-xl bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold text-sm transition-all shadow-lg shadow-indigo-600/30 cursor-pointer disabled:opacity-50"
            >
              {loading ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
